...
let choose_pairs = fix (fun choose_pairs xs ys ->
  ...
  | fresh z zs in
    xs ~ Cons z zs;
    choose_pairs zs (choose z ys))

let digits = (Cons 1 (Cons 2 (Cons 3 (Cons 4 (Cons 5 (Cons 6 (Cons 7 (Cons 8 (Cons 9 Nil)))))))))
...
let rev = fix (fun rev acc xs ->
  | xs ~ Nil; acc
  | fresh x xs' in
    let _ = xs ~ Cons x xs' in
    rev (Cons x acc) xs') Nil

let list_of = fix (fun list_of acc f n ->
  | n ~ Z; rev acc
  | let n' = sub1 n in
    fun x -> (list_of (Cons (f x) acc) f n')) Nil
